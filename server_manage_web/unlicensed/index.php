...
<h2 class="account_title">Tài khoản chưa Cấp quyền</h2>
...
<?php
    $count_account = "../../server/file_licensing/unlicensed/";
    $filecount = 0;
    ...
    foreach (new DirectoryIterator('../../server/file_licensing/unlicensed/') as $fileInfo)
    {
        if($fileInfo->isDot()) continue;
        $account_name = $fileInfo->getFilename();
    
        $user_name = "../../server/account/{$account_name}/login_name";
        $fp_user_name = fopen($user_name, "r");
        $contents_user = fread($fp_user_name, filesize($user_name));
        fclose($fp_user_name);
    
        $password = "../../server/account/{$account_name}/password";
