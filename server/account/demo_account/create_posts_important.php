...
<div id="container">
    <h1>Bài Viết chủ đề Quan Trọng</h1>
...
<?php
    ...
    function post_crt()
    {
        ...
        if(file_exists("all_post/{$title}")) {
            ...
            // Tạo thư mục có tên là tên bài viết trong important
            mkdir("important/{$title}");

            // Tạo phần mô tả
            $write_describe = fopen("important/{$title}/describe", "w+");
            fwrite($write_describe, $describe);

            // Tạo phần content
            $write_content = fopen("important/{$title}/content", "w+");
            fwrite($write_content, $content);

            // Tạo file cài đặt chế độ bài viết là mã hóa
            $write_post_mode = fopen("important/{$title}/post_mode", "w+");
            fwrite($write_post_mode, "encode");
